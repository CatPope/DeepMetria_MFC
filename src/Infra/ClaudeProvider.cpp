                                              ...
                                              const std::wstring& model) const
{
    ...
    // 1) 시스템 프롬프트: 역할 + 데이터 개요 + 컬럼 메타 + 도구 카탈로그 + 응답 형식
    std::wstring sys;
    sys += L"당신은 사용자의 데이터를 분석하고 시각화로 답하는 한국어 데이터 분석 어시스턴트입니다. "
           L"데이터 자체의 행 값은 보이지 않으며 컬럼 메타와 도구만 보고 결정합니다.\n\n";
    sys += L"[데이터 개요] " + req.dataSummary + L"\n";
    if (!req.columns.empty())
    {
        ...
        for (const auto& c : req.columns)
        {
            const wchar_t* t =
                c.typeEnum == 1 ? L"수치" :
                c.typeEnum == 2 ? L"텍스트" :
                c.typeEnum == 3 ? L"날짜" : L"기타";
            sys += L"- " + c.name + L" (" + t + L")";
            if (c.missing > 0)
                sys += L", 결측 " + std::to_wstring(c.missing);
            if (c.typeEnum == 1)  // 수치
            {
                wchar_t buf[128];
                swprintf_s(buf, L", 평균 %.2f, min %.2f, max %.2f",
                           c.meanVal, c.minVal, c.maxVal);
                sys += buf;
            }
            else if (c.typeEnum == 2 && c.uniqueCount > 0)  // 텍스트
            {
                sys += L", " + std::to_wstring(c.uniqueCount) + L"개 고유값";
            }
            if (!c.sampleValue.empty())
                sys += L", 예: \"" + c.sampleValue + L"\"";
            sys += L"\n";
        }
    }
    if (!req.tools.empty())
    {
        sys += L"[사용 가능한 도구] (모두 시각화 생성형 — 호출 시 viz 카드가 만들어짐)\n";
    ...
    sys += L"\n[데이터 형식 가이드]\n"
           ...
           L"[도구 종류 안내] 위 도구는 모두 시각화 생성형입니다. "
           L"호출 시 viz 카드가 만들어집니다. 사용자가 시각화/그래프/표를 명시적으로 요청한 경우에만 호출하세요.\n"
           L"단순 질문/대화/설명 요청에는 toolName 빈 값으로 답변만 합니다.\n"
           L"필드 규칙:\n"
           L"- title: 시각화 카드 헤더에 들어가는 매우 짧은 라벨 (8자 이내, 예: '월별 매출 추이')\n"
           L"- description: ★★ 반드시 포함 (필수) ★★. 사용자에게 채팅으로 전달할 1-3문장의 자연어 설명.\n"
           L"  title과 절대 같으면 안 되고, '시각화를 만들었습니다' 같은 무의미한 안내문 금지.\n"
           L"  실제 데이터에서 발견한 패턴/수치/경향을 짧게 설명할 것.\n"
           L"- reasoning: 도구 선택 사고 흐름 (짧게)\n"
           L"응답 형식 (반드시 JSON):\n"
           L"- 시각화 필요: {\"toolName\":\"...\",\"params\":{...},\"title\":\"짧은 제목\",\"description\":\"결과 설명 문장\",\"reasoning\":\"사고\"}\n"
           L"- 시각화 불필요 (단순 대화): {\"toolName\":\"\",\"description\":\"채팅 답변 (자유 길이)\",\"reasoning\":\"사고\"}\n"
           L"어떤 경우든 description 은 비워두면 안 됨.";

    // 2) messages 배열 구성: 이력 + 마지막 사용자 질문
    std::string msgsJson = "[";
    bool first = true;
    for (const auto& turn : req.history)
    {
        ...
        first = false;
        msgsJson += "{\"role\":\"";
        msgsJson += (turn.isUser ? "user" : "assistant");
        msgsJson += "\",\"content\":\"";
        msgsJson += JsonEscape(WToU8(turn.text));
        msgsJson += "\"}";
    }
    if (!first) msgsJson += ",";
    msgsJson += "{\"role\":\"user\",\"content\":\"";
    msgsJson += JsonEscape(WToU8(req.question));
    msgsJson += "\"}]";

    std::ostringstream body;
    body << "{\"model\":\"" << WToU8(model.empty() ? L"claude-opus-4-7" : model) << "\","
         << "\"max_tokens\":1024,"
         << "\"system\":\"" << JsonEscape(WToU8(sys)) << "\","
         << "\"messages\":" << msgsJson << "}";
    ...
    {
        r.ok = false;
        wchar_t buf[64]; swprintf_s(buf, L"Claude HTTP %d", resp.status);
        r.error = buf;
    ...
    // Claude도 content[0].text 안에 JSON string 으로 포장되어 \" escape 됨. unescape.
    {
    ...
    // text 필드 다음의 '{' 부터 매칭되는 '}' 까지를 inner 로 추출. markdown fence 제거.
    std::wstring inner = w;
    {
    ...
    // 정책: description 은 반드시 LLM 이 제공해야 함. 누락 시 에러.
    if (r.description.empty())
    {
        r.ok = false;
        r.error = L"LLM이 description 필드를 반환하지 않았습니다. (Claude 응답)";
        return r;
    }
    r.ok = true;
    return r;
